          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8 rounded-full grayscale">
                <AvatarImage src={vendor?.data?.hotelDetails?.images[0]?.url || `https://api.dicebear.com/10.x/initials/svg?seed=${vendor?.data?.businessDetails?.businessName}`} alt={vendor?.data?.businessDetails?.businessName || 'vendor'} />
                <AvatarFallback className="rounded-lg">{vendor?.data?.businessDetails?.businessName?.charAt(0).toUpperCase() + vendor?.data?.businessDetails?.businessName?.charAt(1).toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-medium">{vendor?.data?.businessDetails?.businessName}</span>
            ...
            <DropdownMenuLabel className="p-0 font-normal">
              <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                <Avatar className="h-8 w-8 rounded-lg">
                  <AvatarImage src={vendor?.data?.hotelDetails?.images[0]?.url || `https://api.dicebear.com/10.x/initials/svg?seed=${vendor?.data?.businessDetails?.businessName}`} alt={vendor?.data?.hotelDetails?.name || 'vendor'} />
